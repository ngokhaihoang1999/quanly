...
export async function handleGroupChat(update: any) {
  ...
  if (text === '/start' || text.startsWith('/start@') || text === '/menu' || text.startsWith('/menu@')) {
    ...
    // Build dynamic menu
    const miniAppBase = 'https://ngokhaihoang1999.github.io/quanly/mini-app/index.html?v=202606021730';
    ...
    // Nút Xem hồ sơ: dùng t.me deep link mở Mini App trong Telegram
    if (linkedProfileId) {
      keyboard.push([{ text: '👤 Xem hồ sơ Trái quả', url: `https://t.me/quanlyhcm_bot/app?startapp=${linkedProfileId}` }]);
    } else {
