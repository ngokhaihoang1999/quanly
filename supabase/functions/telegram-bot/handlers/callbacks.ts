...
export async function handleCallback(update: any, staffData: any) {
  ...
  if (cbData === 'menu_link_profile') {
    ...
    if (!profiles || profiles.length === 0) return sendText(chatId, `ℹEEChưa có hềEsơ nào ềEgiai đoạn BB cần gắn group.`);
  ...
  if (cbData === 'menu_open_kt') {
    ...
    if (!['bb', 'center', 'completed'].includes(p.phase)) {
      return sendText(chatId, `⚠EEHềEsơ *${p.full_name}* chưa đến giai đoạn BB. Bấm "Xem thông tin Group" đềEkiểm tra.`);
    }
  ...
  if (cbData.startsWith('link_fg_')) {
    ...
    if (!['tu_van', 'bb', 'center', 'completed'].includes(profile.phase)) {
      ...
      return sendText(chatId, `⚠EEHềEsơ *${profile.full_name}* chưa ềEgiai đoạn BB  Ekhông thềEgắn group BB.`);
    }
    await editMessageReplyMarkup(chatId, messageId, null);
    ...
    if (!finalLink) {
      msg += `\n\n⚠EEBot chưa phải là admin (hoặc thiếu quyền Mời người dùng) nên không thềETự Động lấy Link Group.\n\nHãy dùng lệnh sau đềEMini App có thềEmềEGroup:\n\`/setlink [Link_mời_vào_nhóm_này]\``;
    }
    await sendText(chatId, msg);
    return;
  }
  ...
  if (cbData.startsWith('approve_hapja_')) {
    ...
    if (!hapja || hapja.status !== 'pending') { await sendText(chatId, `⚠EEPhiếu không tồn tại hoặc đã xử lý.`); return; }
  ...
  if (isAdmin && cbData.startsWith("approve_")) {
    ...
    if (!ps?.pending_telegram_id) { await sendText(chatId, "⚠EEKhông có yêu cầu pending."); return; }
