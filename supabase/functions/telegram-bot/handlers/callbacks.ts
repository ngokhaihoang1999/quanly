...
export async function handleCallback(update: any, staffData: any) {
  ...
  if (cbData === 'menu_view_report') {
    ...
    if (showSinka) kb.push([{ text: `📜 Thẻ học viên`, callback_data: 'view_sinka' }]);
  ...
  if (cbData === 'view_sinka') {
    await editMessageText(chatId, messageId, `📜 *Thẻ học viên*\nChọn cách xuất:`, [
...
Mở Mini App → tab 📜 Thẻ HV → bấm 📄 Word`,
      [
