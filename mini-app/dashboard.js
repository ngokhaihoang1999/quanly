...
const PHASE_LABELS = {new:'🟡 Chakki',chakki:'🟡 Chakki',tu_van_hinh:'🖼�E�ETV Hình',tu_van:'💬 Tư Vấn',bb:'🎓 BB',center:'🏛�E�ECenter',completed:'✁E};
  ...
  try {
    ...
    if (scope === 'system') {
      ...
      if (myArea) {
        ...
        // Specialist with area scope  Efind area containing my team
    ...
    document.getElementById('dashUnitTitle').textContent = '🏢 ' + (unitLabel || 'Đơn vềE) + semLabel;
    ...
    // ALL profiles in unit (Alive + Dropout)  Eused for cumulative metrics
    const unitProfilesMapAll = new Map();
    // Alive-only profiles map  Eused for phase metrics
    ...
    if (unitStaffCodes.length > 0) {
      ...
      // Approved Hapja by unit members (filtered by semester)  Eused for Hapja cumulative metric
    ...
    // ── Info descriptions ──
    const infoDescs = {
      hapja_cum:   'Tổng sềETrái đã được duyệt Hapja (tích luỹ, tính cả Drop-out)',
      tvh_cum:     'SềETrái đã đạt giai đoạn Tư Vấn Hình trềElên (tích luỹ, tính cả Drop-out)',
      grptv_cum:   'SềEGroup TV (telegram) đã kết nối bot (tích luỹ, tính cả Drop-out)',
      grpbb_cum:   'SềETrái đã vào giai đoạn BB / Đã mềEKT (tích luỹ, tính cả Drop-out)',
      center_cum:  'SềETrái đã vào giai đoạn Center (tích luỹ, tính cả Drop-out)',
      chakki_ph:   'Trái đang ềEgiai đoạn Chakki (đã duyệt Hapja nhưng chưa lên TV Hình)',
      tvh_ph:      'Trái đang ềEgiai đoạn TV Hình (chốt TV lần 2 nhưng chưa lên Group TV)',
      grptv_ph:    'Trái đang ềEgiai đoạn Tư Vấn / Group TV (chưa mềEKT)',
      grpbb_ph:    'Trái đang ềEgiai đoạn BB / đã mềEKinh Thánh',
      center_ph:   'Trái đang ềEgiai đoạn Center',
    };
    ...
    function renderDashMetrics() {
      ...
      // ══╁ETÍCH LUỸ (Alive + Dropout) ══╁E
      // 1. Hapja (chềEA mới tính từ approved hapja; B không có hapja riêng  Ehiển thềE0)
      ...
      // Store for popup  Ealso keep cumHapja in window for persistence
      ...
      metricsEl.innerHTML = `
        <div style="display:flex;gap:6px;margin-bottom:6px;">
          <div class="dash-mode-toggle" style="margin-bottom:0;flex:1;">
            ...
            <button class="dash-mode-btn ${!isA ? 'active' : ''}" onclick="window._dashScopeMode='B';renderDashMetrics()">🤁EHềEtrợ TV/BB (B)</button>
          </div>
        </div>
    ...
    // Pre-fetch latest records AND sessions per profile  Eboth needed for latestActivityLabel()
    ...
    function fruitCardsForCodes(codes) {
      ...
      return matching.map(r => {
        ...
        const ndd = allR.find(x=>x.role_type==='ndd')?.staff_code || p?.ndd_staff_code || ' E;
        const tvv = allR.filter(x=>x.role_type==='tvv').map(x=>x.staff_code).join(', ') || ' E;
        const gvbb = allR.find(x=>x.role_type==='gvbb')?.staff_code || ' E;
        const latest = latestActivityLabel(recordMap[pid], sessionMap[pid]);
        const fStatus = p?.fruit_status || 'alive';
        const sDot = fStatus === 'dropout' ? '🔴' : fStatus === 'pause' ? '⏸�E�E : '🟢';
        
        const isKT = p?.is_kt_opened;
        const showKT = ['bb', 'center', 'completed'].includes(ph);
        const ktLabel = showKT ? `<span style="font-size:9px;font-weight:700;padding:2px 7px;border-radius:8px;background:${isKT ? 'var(--green)' : '#f59e0b'};color:white;margin-left:4px;">${isKT ? '📖 Đã mềEKT' : '📕 Chưa mềEKT'}</span>` : '';
    ...
    function countForCodes(codes) {
      // ChềEđếm hềEsơ Alive mà NDD trực thuộc codes (không tính TVV/GVBB từ đơn vềEkhác)
          ...
          subHtml += `<div style="margin-bottom:6px;">
            <div class="subunit-group-hd" onclick="document.getElementById('${gid}').style.display=document.getElementById('${gid}').style.display==='none'?'block':'none'">
              ...
              <div style="display:flex;gap:12px;font-size:11px;color:var(--text2);">
                <span>${gS.td} TāE/span><span style="color:var(--accent);font-weight:700;">${gS.fruits} 🍎</span>
              </div>
            </div>
            ...
            subHtml += `<div style="margin:4px 0;">
              <div class="subunit-team-hd" onclick="document.getElementById('${tid}').style.display=document.getElementById('${tid}').style.display==='none'?'block':'none'">
                ...
                <div style="display:flex;gap:10px;font-size:11px;color:var(--text2);">
                  <span>${tS.td} TāE/span><span style="color:var(--green);font-weight:700;">${tS.fruits} 🍎</span>
                </div>
              </div>
          ...
          sortedTeams2.forEach(t => {
            ...
            subHtml += `<div style="margin-bottom:4px;">
              <div onclick="document.getElementById('${tid}').style.display=document.getElementById('${tid}').style.display==='none'?'block':'none'" style="cursor:pointer;display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:var(--surface2);border-radius:var(--radius-sm);border:1px solid var(--border);">
                ...
                <div style="display:flex;gap:10px;font-size:11px;color:var(--text2);">
                  <span>${tS.td} TāE/span><span style="color:var(--accent);">${tS.fruits} 🍎</span>
                </div>
              </div>
              <div id="${tid}" style="display:none;padding-left:12px;">${fruitCardsForCodes(tCodes)}</div>
            </div>`;
    ...
    subEl.innerHTML = subHtml ? `<div class="section-header" style="margin-top:8px;margin-bottom:6px;"><div class="section-title" style="font-size:13px;">📊 Đơn vềEcấp dưới</div></div>${subHtml}` : '';
    ...
    if (hapjas.length === 0) {
      ...
      hList.innerHTML = hapjas.map(h => {
        ...
        // ⚠�E�EWarning icon for revision status  Eclick to show feedback popup
        let warnIcon = '';
        if (h.status === 'revision' && h.feedback) {
          ...
          warnIcon = `<span onclick="event.stopPropagation();showHapjaFeedback('${escapedFb}','${fbBy}','${fbDate}')" style="font-size:18px;cursor:pointer;animation:pulse 1.5s infinite;margin-right:4px;" title="Cần chỉnh sửa  Ebấm xem">⚠�E�E/span>`;
        }
    ...
    stuck.forEach(id => {
      ...
      if (el) el.innerHTML = '<div style="text-align:center;padding:12px;color:var(--text3);font-size:13px;">⚠�E�EKhông tải được dữ liệu</div>';
    });
    ...
    if (personalEl && !personalEl.innerHTML.trim()) personalEl.innerHTML = '<div style="text-align:center;padding:12px;color:var(--text3);font-size:13px;">⚠�E�EKhông tải được dữ liệu</div>';
  } finally {
    markFresh('dashboard');
    // GUARANTEE: After loadDashboard completes (success or error), any element still
    // showing "Đang tải..." means it was never updated  Ereplace with empty/error state
    const LOADING_MARKER = 'Đang tải';
    [
      { id: 'dashHapjaList', fallback: '<div style="text-align:center;padding:12px;color:var(--text3);font-size:13px;">Không có phiếu chềEduyệt</div>' },
...
window.renderPersonalList = async function(type) {
  ...
  const titles = {
    ...
    managed: '🛡�E�ETổng hềEsơ được quyền quản lý'
  };
  titleEl.textContent = titles[type] || 'Danh sách';
  
  if (listData.length === 0) {
    if (typeof showSkeleton === 'function') showSkeleton(listEl, 2);
  else listEl.innerHTML = '<div style="text-align:center;padding:12px;color:var(--text2);font-size:13px;">Chưa có trái nào</div>';
    return;
  }
  
  // Show skeleton loading state
  if (typeof showSkeleton === 'function') showSkeleton(listEl, 2);
  else listEl.innerHTML = '<div style="text-align:center;padding:12px;color:var(--text3);font-size:13px;">Đang tải...</div>';
